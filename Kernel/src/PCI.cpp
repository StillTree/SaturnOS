...
static auto MapEntireBus(PhysicalAddress baseAddress, U8 bus) -> Result<void>
{
	...
	for (USIZE i = 0; i < 256; i++) {
		auto result = page.MapTo(frame, PageTableEntryFlags::Present);
		if (result.IsError()) {
			return result;
		}
...
static auto UnmapEntireBus(VirtualAddress baseAddress, U8 bus) -> Result<void>
{
...
static auto EnumerateDevices(const MCFG::Entry& segmentGroup, U8 bus) -> USIZE
{
	USIZE deviceCount = 0;

	for (U8 device = 0; device < 32; device++) {
		for (U8 function = 0; function < 8; function++) {
			VirtualAddress configSpaceAddress(segmentGroup.BaseAddress + (static_cast<U64>(bus) << 20) + (static_cast<U64>(device) << 15)

...
auto BitmapFrameAllocator::Init(MemoryMapEntry* memoryMap, USIZE memoryMapEntries) -> Result<void>
{
	...
	const Frame<Size4KiB> lastFrame(memoryMap[memoryMapEntries - 2].PhysicalEnd.Value + 1);
	...
	if (neededFrames >= ((memoryMap[0].PhysicalEnd.Value + 1 - memoryMap[0].PhysicalStart.Value) / Frame<Size4KiB>::SIZE_BYTES)) {
	...
	m_frameBitmap = memoryMap[0].PhysicalStart.AsPointer<U8>();
	...
	m_lastFrame = Frame<Size4KiB>(m_memoryMap[m_memoryMapEntries - 2].PhysicalEnd.Value);
	...
	for (USIZE i = 0; i < memoryMapEntries - 2; i++) {
		const Frame<Size4KiB> lastFrame(m_memoryMap[i].PhysicalEnd.Value);
		Frame<Size4KiB> frame(m_memoryMap[i].PhysicalStart.Value);
